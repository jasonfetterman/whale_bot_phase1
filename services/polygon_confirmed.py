...
async def handle_polygon_block(bot: Bot, head: dict):
    ...
    threshold = await get_for_user(
        ...
        default=settings.POLYGON_WHALE_THRESHOLD,
    )
    ...
    for tx in txs:
        try:
            ...
            value_matic = int(value_hex, 16) / 1e18
            if value_matic < threshold:
                continue
            ...
            importance = score_importance(value_matic, threshold)
...
handle_block = handle_polygon_block
